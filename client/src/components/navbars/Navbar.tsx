...
import { Home, Bell, MessageCircle, User, Pencil, LogOut, ChevronDown } from "lucide-react";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
const Navbar = () => {
  ...
  // สมมติว่าใน Store มีชื่อผู้ใช้ ถ้าไม่มีให้ใช้ค่า Default ไปก่อน
  const user = { name: "นมศักดิ์ มาโล", role: "นักศึกษา" };

  return (
    <nav className="fixed top-0 left-0 right-0 z-50 h-16 bg-gradient-to-r from-[#FFB800] via-[#FF8A00] to-[#FF5800] shadow-lg px-4 md:px-8 flex items-center justify-between">
        ...
        <DropdownMenu modal={false}>
          <DropdownMenuTrigger asChild>
            <button className="group flex items-center gap-2 pl-1 pr-3 py-1 bg-white/10 hover:bg-white/20 rounded-full transition-all border border-white/20">
              ...
              <ChevronDown size={14} className="text-white opacity-70 group-hover:opacity-100" />
            </button>
          </DropdownMenuTrigger>
          <DropdownMenuContent align="end" className="w-56 mt-2 p-2 rounded-2xl shadow-xl border-gray-100">
